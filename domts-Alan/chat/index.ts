...
function createLoader(): HTMLLIElement {
    const li = document.createElement("li")
    li.classList.add("loading")

    const i = document.createElement("i")
    const h4 = document.createElement("h4")
    h4.textContent = "Cargando..."
    const h5 = document.createElement("h5")
    h5.textContent = "Esto puede tardar un poco. Paciencia."
...
function createSmall(): HTMLElement {
...
function createMessage(text: string, sender: Sender): HTMLLIElement {
    const li = document.createElement("li")
...
const engine = await CreateWebWorkerMLCEngine(
    ...
    SELECTED_MODEL,
    {
        initProgressCallback: (progressInfo: { text: string; progress: number }) => {
