...
function createButtonEvP(
    ...
    id: string,
    className: string,
    options: { event: string; handler: () => void }
): HTMLButtonElement {
...
function injectHTML(content: string): void {
    const app = document.getElementById("App");
    if (app) {
...
const button2 = createInjectorButton(
    "2 Inyectar HTML",
...
button2.addEventListener("click", () => {
    ...
    if (appDiv) {
        ...
        const button1 = createInjectorButton(
            "Inyectar HTML",
...
if (appDiv) {
    ...
    const b = createInjectorButton(
        "Botón A",
        "<p>Componente A dentro de #App</p>"
    );
    const button1 = createInjectorButton(
        "Botón B",
        "<p>Componente B dentro de #App</p>"
    );
...
if (appDiv) {
    ...
    appDiv.innerHTML = "";
    const b = createInjectorButton(
        "Botón A (sin duplicar)",
        "<p>Contenido único dentro de #App</p>"
    );
    appDiv.appendChild(b);
}
...
if (appDiv) {
    ...
    const buttonC = createInjectorButton(
        "Botón C",
        "<div style='background-color: lightblue; padding: 10px;'>Componente C</div>"
    );

    buttonC.addEventListener("click", () => {
        const buttonB = createInjectorButton(
            "Botón B",
            "<div style='background-color: lightgreen; padding: 10px;'>Componente B</div>",
            appDiv
        );

        buttonB.addEventListener("click", () => {
            const buttonA = createInjectorButton(
                "Botón A",
                "<div style='background-color: lightcoral; padding: 10px;'>Componente A</div>",
                appDiv
            );
